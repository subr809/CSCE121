...
Image::Image(const Image& oldImage) : image(nullptr), width(0), height(0){
  filename = oldImage.filename;
  allocateImage(oldImage.width, oldImage.height);
  for (unsigned int i = 0; i < width; i++) {
		for (unsigned int j = 0; j < height; j++) {
			this->image[i][j].setRed(oldImage.image[i][j].getRed());
			this->image[i][j].setGreen(oldImage.image[i][j].getGreen());
			this->image[i][j].setBlue(oldImage.image[i][j].getBlue());
		}
  }
